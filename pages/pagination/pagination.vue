<template>
	<view class="content">
		<view class="text-area">
			...
			<view v-for="item in ListPage" :key='item.id'>
				<view class="list-img">
					<view>
						<image style="width: 120upx;height: 120upx;" :src="item.img"/>
					</view>
	...
	export default {
		...
		methods: {
			init(){
				...
				this.ListPage = [];
				this.pagination.total = this.pageList.length;
				for(let i=0;i<this.pagination.pageSize&& i<this.pageList.length;i++){
			...
			changePage(e){
				this.ListPage = [];
				this.pagination.total = this.pageList.length;
				for(let i=(e.current - 1) * this.pagination.pageSize;i<((e.current - 1) * this.pagination.pageSize + this.pagination.pageSize) && i<this.pageList.length;i++){
					this.ListPage.push(this.pageList[i]);
				}
